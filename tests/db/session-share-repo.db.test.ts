...
async function createUser(db: DatabaseProvider, email: string, nickname: string) {
  return db.users.create({ email, password_hash: 'h', nickname, role: 'team' });
}

async function createSession(db: DatabaseProvider, userId: string, title = 'Test Session') {
  return db.sessions.create(title, 'team', userId);
}
...
describe('Session Share Repository', () => {
  beforeEach(async () => {
    ...
    // The DB project supplies a clean transaction and rolls it back after the test.
  });
  ...
  it('creates shares for specific users', async () => {
    ...
    await db.sessionShares.createMany([{ session_id: session.id, shared_with: bob.id, shared_by: alice.id }]);
  ...
  it('counts team shares as unread for any user', async () => {
    ...
    await db.sessionShares.createMany([{ session_id: session.id, shared_with: '__team__', shared_by: alice.id }]);
  ...
  it('orders unread first, then by created_at desc', async () => {
    ...
    expect(sorted[0].read_at).toBeNull();
    expect(sorted[0].session_id).toBe(s2.id);
    // Read share comes second
    expect(sorted[1].read_at).not.toBeNull();
  ...
  it('respects limit and offset', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      await db.sessionShares.createMany([{ session_id: s.id, shared_with: bob.id, shared_by: alice.id }]);
    }
  ...
  it('does not list shares intended for other users', async () => {
    ...
    await db.sessionShares.createMany([{ session_id: session.id, shared_with: carol.id, shared_by: alice.id }]);
  ...
  it('refuses read receipts for sessions outside the caller inbox', async () => {
    const alice = await createUser(db, 'alice@test.com', 'Alice');
    const bob = await createUser(db, 'bob@test.com', 'Bob');
    const carol = await createUser(db, 'carol@test.com', 'Carol');
    const session = await createSession(db, alice.id);
    await db.sessionShares.createMany([{ session_id: session.id, shared_with: carol.id, shared_by: alice.id }]);
    const [share] = await db.sessionShares.getSharesForSession(session.id);

    await expect(db.sessionShares.markReadForUser(share!.id, bob.id)).resolves.toBe(false);
    await expect(db.sessionShares.markAllReadInSession(bob.id, session.id)).resolves.toBe(false);
    expect(await db.sessionShares.countUnread(carol.id)).toBe(1);
  });
  ...
  it('markAllRead marks every unread share for a user as read', async () => {
    ...
    expect(listed.every((s) => s.read_at !== null)).toBe(true);
  });
  ...
  it('deletes a single share by id', async () => {
    ...
    await db.sessionShares.deleteOne(bobShare.id, session.id);
  ...
  it('does not delete a share when its parent session does not match', async () => {
    const alice = await createUser(db, 'alice@test.com', 'Alice');
    const bob = await createUser(db, 'bob@test.com', 'Bob');
    const firstSession = await createSession(db, alice.id, 'First');
    const secondSession = await createSession(db, alice.id, 'Second');

    await db.sessionShares.createMany([{ session_id: firstSession.id, shared_with: bob.id, shared_by: alice.id }]);
    const [share] = await db.sessionShares.getSharesForSession(firstSession.id);

    await expect(db.sessionShares.deleteOne(share.id, secondSession.id)).resolves.toBe(false);
    expect(await db.sessionShares.getSharesForSession(firstSession.id)).toHaveLength(1);
  });
  ...
  // ─── Deleting the session takes its shares with it ─────

  it('deleting a session clears its shares, reads and unread badge', async () => {
    const alice = await createUser(db, 'alice@test.com', 'Alice');
    const bob = await createUser(db, 'bob@test.com', 'Bob');
    const carol = await createUser(db, 'carol@test.com', 'Carol');
    const session = await createSession(db, alice.id);
    ...
    await db.sessionShares.markAllReadInSession(carol.id, session.id);
    // Two rows, because Bob matches both the direct share and the team one —
    // countUnread counts share rows, not conversations.
    expect(await db.sessionShares.getSharesForSession(session.id)).toHaveLength(2);
    expect(await db.sessionShares.countUnread(bob.id)).toBe(2);

    await db.sessions.delete(session.id);

    // Before the FK cascade the share rows outlived the session, so this count
    // stayed at 1 forever: a badge pointing at a conversation that is gone.
    expect(await db.sessionShares.countUnread(bob.id)).toBe(0);
    expect(await db.sessionShares.listForUser(bob.id)).toHaveLength(0);
    expect(await db.sessionShares.getSharesForSession(session.id)).toHaveLength(0);
    expect(await db.sessionShares.getSharedSessionIds(bob.id)).not.toContain(session.id);
  });
});
