 ...
 *
 * Runs inside the transaction-isolated DB project (one rollback per test).
 */
...
describe('Workspace settings service', () => {
  ...
  it('overwrites on conflict — including switching columns', async () => {
    ...
    expect((await db.workspaceSettings.list()).filter((r) => r.key === 'llm.model').length).toBe(1);
  });

  it('stores structured json values', async () => {
    const tokens = { brand: '#2e8b3d', light: { '--t-surface': '#ffffff' } };
    await db.workspaceSettings.set('branding.theme_tokens', { value: tokens });
    const row = await db.workspaceSettings.get('branding.theme_tokens');
    expect(row?.value).toEqual(tokens);
  });
  ...
  it('lists rows ordered by key', async () => {
    ...
    expect(keys.indexOf('branding.product_name')).toBeLessThan(keys.indexOf('search.tavily_api_key'));
  });
});
