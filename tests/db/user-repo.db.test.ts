...
describe('User Repository', () => {
  ...
  it('lists all users', async () => {
    // The surrounding transaction hides this test's writes from everyone else,
    // but not the other way round: `list()`/`count()` still see whatever any
    // other connection has committed. So assert about our own rows only —
    // absolute table state belongs to no single test.
    const scope = randomUUID();
    await db.users.create({ email: `a-${scope}@test.com`, password_hash: 'h', nickname: 'A', role: 'team' });
    await db.users.create({ email: `b-${scope}@test.com`, password_hash: 'h', nickname: 'B', role: 'super' });
    await db.users.create({ email: `c-${scope}@test.com`, password_hash: 'h', nickname: 'C', role: 'super' });

    const users = await db.users.list();
    const created = users.filter((user) => user.email.includes(scope));
    expect(created.map((user) => user.nickname).sort()).toEqual(['A', 'B', 'C']);
  });
  ...
  it('counts users', async () => {
    ...
    await db.users.create({ email: `a-${scope}@test.com`, password_hash: 'h', nickname: 'A', role: 'team' });
    await db.users.create({ email: `b-${scope}@test.com`, password_hash: 'h', nickname: 'B', role: 'team' });

    expect(await db.users.count()).toBe(before + 2);
  });
