...
/**
 * AND→OR top-up in search().
 *
 * The FTS runs two passes: AND (every token must match — precision) then OR
 * (any token — recall). It used to RETURN on the first non-empty pass, so a
 * single mediocre AND hit hid the entire OR candidate set. Now AND results come
 * first and OR fills the remaining slots, deduped by id.
 */
describe('Knowledge Base Repository — search() AND→OR top-up', () => {
  beforeEach(async () => {
    db = await initDatabase({ type: 'pg', pgConnectionString: TEST_DATABASE_URL });
  });
  ...
  /** Unique per run so these rows never collide with a parallel worker's. */
  const tag = () => `kbor${Date.now()}${Math.floor(process.hrtime()[1] / 1000)}`;

  async function seedDoc(owner: string, docId: string, title: string, content: string) {
    return db.knowledgeBase.create({
      doc_id: docId,
      scope: 'shared',
      title,
  ...
  it('returns AND matches first, then tops up from the OR pass', async () => {
    const u = await createUser(db, `or-${tag()}@test.com`, 'Or');
    const t = tag();
    // Only `both` contains BOTH tokens → the AND pass returns exactly one row.
    await seedDoc(u.id, `${t}-both`, `${t}alpha ${t}beta`, 'both tokens present');
    await seedDoc(u.id, `${t}-alpha`, `${t}alpha only`, 'first token only');
    await seedDoc(u.id, `${t}-beta`, `${t}beta only`, 'second token only');

    const hits = await db.knowledgeBase.search(`${t}alpha ${t}beta`, { visibility: 'team', limit: 10 });
    const ids = hits.map((h) => h.doc_id);

    // Old behaviour stopped at the single AND hit; all three are reachable now.
    expect(ids).toContain(`${t}-both`);
    expect(ids).toContain(`${t}-alpha`);
    expect(ids).toContain(`${t}-beta`);
    // Precision still leads: the doc matching every token ranks first.
    expect(ids[0]).toBe(`${t}-both`);
    // The AND hit appears once, not twice (the OR pass re-finds it).
    expect(ids.filter((id) => id === `${t}-both`)).toHaveLength(1);
  });

  it('never exceeds the requested limit while topping up', async () => {
    const u = await createUser(db, `orlim-${tag()}@test.com`, 'OrLim');
    const t = tag();
    await seedDoc(u.id, `${t}-both`, `${t}gamma ${t}delta`, 'both');
    for (let i = 0; i < 5; i++) await seedDoc(u.id, `${t}-g${i}`, `${t}gamma ${i}`, 'one token');

    const hits = await db.knowledgeBase.search(`${t}gamma ${t}delta`, { visibility: 'team', limit: 2 });
    expect(hits).toHaveLength(2);
    expect(hits[0].doc_id).toBe(`${t}-both`);
  });
});
