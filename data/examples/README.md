...
- Files load in a fixed FK-safe order (see `LOAD_ORDER` in
  [`apps/api/src/cli/commands/seed.ts`](../../apps/api/src/cli/commands/seed.ts)).
...
- **Secrets are never baked in.** `users.json` rows carry a plaintext `password`
  ...
  `email_accounts`) are intentionally **not** seeded.
...
**Identity & access** — 5 users (super / team / disabled), 2 groups + membership.
...
**Chat** — 6 sessions across the `web` and `subagent` channels, showing
tool pipelines (`knowledge_query`, `external_search`, `project_query`,
...
- `api_clients`, `email_accounts` — values are hashed/encrypted
