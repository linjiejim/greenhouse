...
export const driveFolders = pgTable(
  'drive_folders',
  {
    ...
    scope: text('scope', { enum: ['kb', 'tables'] }).notNull(),
  ...
  (table) => [
    ...
    check(
      ...
      sql`(
        (${table.scope} = 'kb' AND ${table.base_id} IS NULL AND ${table.visibility} IS NOT NULL AND (${table.visibility} = 'team' OR ${table.owner_user_id} IS NOT NULL))
        OR (${table.scope} = 'tables' AND ${table.base_id} IS NOT NULL AND ${table.visibility} IS NULL AND ${table.owner_user_id} IS NULL)
      )`,
    ),
  ],
);
