...
export const myTool = defineTool({ meta, kind: 'static', create: () => createMyTool() });
```

- 参数用 Zod schema，每个字段加 `.describe()`
- `description` 就用 `meta.description`（单一来源，与 `inputSchema` / `execute` 同处）
- `kind` 必填。`static` 只构造一次，`create` 收到共享的 `db`
- 需要知道调用方（user / session）的工具用 lazy：`defineTool({ meta, kind: 'lazy' })`，不带 `create`；
  同文件导出工厂（如 `createMyTool(db, { userId })`），再到 `agent-runtime/tool-resolution.ts` 的
  `buildLazyServerTools` 按 id 加一个构造分支，会话绑定的工具放进 `if (sessionId)` 块（proxy / MCP 没有会话，
  所以永远拿不到它们）。⚠️ 通用的 `createLazy(ctx)` 路径**只遍历扩展工具**：core 工具写了 `createLazy`
  会进目录、却永远不被构造
...
| `unattendedReplaySafe: true` | 可在无人值守时运行、租约过期后可重放（比只读 proxy 更严：付费调用、会落持久对象的读默认 false） |
...
- 护栏测试会因新工具失败，属预期，按真实意图同步改：`tools/__tests__/surface-derivation.test.ts`
  钉住 core 工具每个派生集合的精确成员（proxy 读 / 写、MCP 分组、workbench、lazy id）；
  `description-budget.test.ts` 卡工具描述总长（先压缩描述，抬预算要在同一个 PR 写理由）
...
- [ ] `meta` 与实现同文件，经 `defineTool({ meta, kind, create? })` 导出；lazy 工具已在 `buildLazyServerTools` 接好构造分支
