...
- **同事收件箱入口**：full Chat 顶部持续显示 `chat/agent-avatar-picker.tsx` 圆形头像，点击导航到各自最后话题，不能改写现有 session 的身份；标题行保留头像在前。ResizeObserver 控制溢出到“更多”；排序复用 `useListReorder`（鼠标、触屏长按）并按用户保存，也支持 Alt + 方向键。头像映射统一在 `chat/profile-avatar.ts`。默认 Sprouty 的模型选择仍在 Composer；自定义 Agent 使用其配置的 model_id。共享 Agent 首版只有分别私聊。
- **连续记录与独立上下文**：`use-coworker-inbox.ts` 读取当前用户的持久收件箱、分页话题及 Runtime 活动，通过既有 WS 失效通知与可见页面轮询刷新；`coworker-history.tsx` 汇集话题摘要，展开仍使用唯一的 ConversationPane。“新话题”保留历史和长期记忆，但不传入其他话题消息。我的侧栏按同事过滤，完整历史/共享/团队视图仍可显式进入。SessionManager 持有后台流，导航不调用 cancel。
- **已读与恢复**：`use-coworker-reading.ts` 只上报前台聚焦且进入视口的 assistant message IDs；选择头像/打开话题不能直接清空未读，待处理审批独立保留。标题旁历史按钮和“首条未读”可回到旧记录。按用户与话题保存草稿文本及滚动位置到 sessionStorage；恢复需等待话题摘要布局，卸载后的零高度不能覆盖阅读位置。其他附件/引用继续复用原有草稿缓存。
...
- 桌面 placeholder 只声明当前真的可用的入口（`@ Agent` / `/ Task`），移动端只保留「输入消息」短文案；不能让已有会话显示不可用的 `@` 能力。移动端 Composer 的发送/停止按钮固定在 textarea 右下角，Agent 选择只在新会话顶部，底部工具条保留模型与附件。语音输入能力已完整移除，任何 Surface 都不得重新声明或渲染该入口。
...
- **同事讨论**：`agent_chat` 产出的 `agent_dialogue` 由 `tool-call/agent-dialogue-card.tsx` 展示真实往返轮次。同一消息只渲染每个 dialogue 最新快照；保留失败/中断状态，不把同事讨论渲染成人类身份切换。

- 手机已有会话的侧栏、标签、评分和分享收进“会话操作”，为固定头像与标题保留空间；同事讨论正文复用 RichMarkdown，无确认动作回调，不能在同事记录里执行写入。
