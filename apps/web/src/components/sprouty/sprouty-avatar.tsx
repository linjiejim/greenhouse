 ...
 *
 * v2: Accessories rendering (hats, glasses, held items) and leaf styles.
 */
...
function drawWateringCan(ctx: CanvasRenderingContext2D, cx: number, cy: number, s: number) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(0.08);
  ctx.fillStyle = '#5eb8d6';
  ctx.strokeStyle = '#307f98';
  ctx.lineWidth = 1.5 * s;
  roundRect(ctx, -9 * s, -7 * s, 18 * s, 15 * s, 4 * s);
  ctx.fill();
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(0, -7 * s, 7 * s, PI, TAU);
  ctx.strokeStyle = '#307f98';
  ctx.lineWidth = 2 * s;
  ctx.stroke();
  ...
  ctx.strokeStyle = '#8adff0';
  ctx.lineWidth = 1.5 * s;
  for (let i = 0; i < 3; i++) {
    ctx.beginPath();
    ctx.moveTo((-24 - i * 2) * s, (-8 + i * 3) * s);
    ctx.lineTo((-29 - i * 2) * s, (-11 + i * 3) * s);
    ctx.stroke();
  }
  ctx.restore();
}
...
function drawBook(ctx: CanvasRenderingContext2D, cx: number, cy: number, s: number) {
  ...
  ctx.beginPath();
  ctx.moveTo(0, -8 * s);
  ctx.quadraticCurveTo(-8 * s, -13 * s, -16 * s, -9 * s);
  ctx.lineTo(-15 * s, 10 * s);
  ctx.quadraticCurveTo(-7 * s, 6 * s, 0, 11 * s);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
  ...
  ctx.strokeStyle = '#c7d6b6';
  ctx.lineWidth = 1 * s;
  for (let i = 0; i < 3; i++) {
    ctx.beginPath();
    ctx.moveTo(-12 * s, (-4 + i * 5) * s);
    ctx.lineTo(-3 * s, (-2 + i * 5) * s);
    ctx.moveTo(3 * s, (-2 + i * 5) * s);
    ctx.lineTo(12 * s, (-4 + i * 5) * s);
    ctx.stroke();
  }
  ctx.restore();
}

function drawBriefcase(ctx: CanvasRenderingContext2D, cx: number, cy: number, s: number) {
  ctx.save();
  ctx.fillStyle = '#b77948';
  ctx.strokeStyle = '#70462d';
  ctx.lineWidth = 1.5 * s;
  roundRect(ctx, cx - 14 * s, cy - 7 * s, 28 * s, 19 * s, 3 * s);
  ctx.fill();
  ctx.stroke();
  ...
  ctx.quadraticCurveTo(cx, cy - 15 * s, cx + 6 * s, cy - 12 * s);
  ctx.lineTo(cx + 6 * s, cy - 7 * s);
  ctx.stroke();

  ctx.strokeStyle = '#e3b274';
  ctx.beginPath();
  ctx.moveTo(cx - 13 * s, cy + 1 * s);
  ctx.lineTo(cx + 13 * s, cy + 1 * s);
  ctx.stroke();
  roundRect(ctx, cx - 2 * s, cy - 1 * s, 4 * s, 5 * s, 1 * s);
...
function drawSprouty(
  ...
  const drawCompactBrandLeaf = (leafStyleOpt === undefined || leafStyleOpt === 'normal') && !opts.bigLeaf;
  ...
  ctx.lineWidth = drawCompactBrandLeaf ? 3 * s : 4 * s;
  ctx.lineCap = 'round';

  if (drawCompactBrandLeaf) {
    // Three compact leaves echo the Greenhouse logo without turning the sprout into
    // a tall antenna. The outer stems fan from one short shared trunk.
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(-1 * s + lWave * 0.08, -9 * s + lDroop * 0.16, 1 * s, -17 * s + lDroop * 0.5);
    ctx.moveTo(0, -7 * s);
    ctx.quadraticCurveTo(-5 * s + lWave * 0.06, -10 * s, -10 * s, -12 * s + lDroop * 0.3);
    ctx.moveTo(0, -8 * s);
    ctx.quadraticCurveTo(5 * s + lWave * 0.06, -12 * s, 10 * s, -14 * s + lDroop * 0.35);
    ctx.stroke();

    leafShape(ctx, -10 * s, -13 * s + lDroop * 0.3, 14 * s, 9 * s, -0.45 + lWave * 0.006, CC.leaf1, CC.leaf2);
    leafShape(ctx, 10 * s, -15 * s + lDroop * 0.35, 14 * s, 9 * s, 0.45 - lWave * 0.006, CC.leaf1, CC.leaf2);
    ellipse(ctx, 1 * s, -21 * s + lDroop * 0.55, 5 * s, 7 * s, CC.leaf1, CC.leaf2, 1.5);
  } else {
    // Keep alternate silhouettes close to the same compact head anchor as
    // Normal. Their personality comes from leaf scale/count, not a long stem.
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(-2 * s + lWave * 0.2, -13 * s + lDroop * 0.3, 2 * s + lWave * 0.15, -23 * s + lDroop);
    ctx.stroke();
    ...
    leafShape(ctx, -10 * s * lf, -15 * s + lDroop * 0.4, 14 * s * lf, 7 * s * lf, -0.5 + lWave * 0.01, CC.leaf3, null);
    leafShape(
      ctx,
      14 * s * lf,
      -18 * s + lDroop * 0.5,
    ...
    leafShape(ctx, 12 * s * lf, -18 * s + lDroop * 0.5, 14 * s * lf, 7 * s * lf, 0.4 - lWave * 0.01, CC.leaf3, null);
    ellipse(ctx, 2 * s, -25 * s + lDroop, 5 * s * lf, 6 * s * lf, CC.leaf1, CC.leaf2, 1.5);
  }

  // Double leaf: extra pair on opposite side
  if (drawDoubleLeaf) {
    leafShape(ctx, 16 * s, -13 * s + lDroop * 0.3, 18 * s, 10 * s, 0.6 - lWave * 0.01, CC.leaf1, CC.leaf2);
    leafShape(ctx, -18 * s, -20 * s + lDroop * 0.6, 18 * s, 10 * s, -0.6 + lWave * 0.01, CC.leaf1, CC.leaf2);
  }

  if (opts.flower) {
    const bloom = 0.6 + Math.sin(t * 2) * 0.4;
    const flowerCenterY = drawCompactBrandLeaf ? -21 : -25;
    const flowerRadius = drawCompactBrandLeaf ? 6 : 8;
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * TAU - PI / 2;
      ellipse(
        ctx,
        2 * s + Math.cos(a) * flowerRadius * s,
        flowerCenterY * s + lDroop + Math.sin(a) * flowerRadius * s,
        6 * s,
        8 * s,
    ...
    ellipse(ctx, 2 * s, flowerCenterY * s + lDroop, 5 * s, 5 * s, '#f8e858', null);
  }
  ctx.restore();
  ...
  function drawProfileEye(ex: number, ey: number, isRight: boolean) {
    if (eyeStyleOpt === 'dot') {
      ellipse(ctx, ex, ey + 1 * s, 5 * s, 7 * s, CC.eyeB, null);
      ellipse(ctx, ex - 1.5 * s, ey - 2 * s, 1.5 * s, 1.8 * s, '#fff', null);
      return;
    }
    if (eyeStyleOpt === 'soft') {
      ellipse(ctx, ex, ey, 8.5 * s, 11 * s, CC.eyeB, null);
      ellipse(ctx, ex - 2.5 * s, ey - 4 * s, 2.6 * s, 3 * s, '#fff', null);
      ellipse(ctx, ex + 2.5 * s, ey + 3 * s, 1.4 * s, 1.4 * s, 'rgba(255,255,255,0.55)', null);
      return;
    }
    if (eyeStyleOpt === 'focused') {
      ellipse(ctx, ex, ey + 2 * s, 10 * s, 7 * s, CC.eyeW, '#5D4037', 2 * s);
      ellipse(ctx, ex, ey + 3 * s, 4.5 * s, 5 * s, CC.eyeB, null);
      ellipse(ctx, ex - 1.5 * s, ey - 1 * s, 1.7 * s, 1.7 * s, '#fff', null);
      const outerX = ex + (isRight ? 7 : -7) * s;
      const innerX = ex + (isRight ? -7 : 7) * s;
      bezierCurve(
        ctx,
        [
          [outerX, ey - 8 * s],
  ...
  if (accessories?.includes('watering-can')) {
    drawWateringCan(ctx, heldLx - 9 * s, heldY - 2 * s, s);
  }
  if (accessories?.includes('book')) {
    drawBook(ctx, heldRx + 8 * s, heldY - 3 * s, s);
  }
  if (accessories?.includes('briefcase')) {
    drawBriefcase(ctx, heldLx - 10 * s, heldY - 1 * s, s);
  }
...
export interface SproutyAvatarProps {
  ...
  /** Leaf style */
  leafStyle?: LeafStyle;
  /** Baseline eye design; emotional states may temporarily override it */
  eyeStyle?: EyeStyle;
  size?: SproutySize;
  animate?: boolean;
  className?: string;
...
export function SproutyAvatar({
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      drawSprouty(ctx, canvasSize, canvasSize, timeRef.current, finalOpts, colorSet, accessories, leafStyle, eyeStyle);
      if (animate) {
  ...
  }, [canvasSize, finalOpts, colorSet, animate, accessories, leafStyle, eyeStyle]);
