 ...
 * new accounts may use the active internal roles `super | team`.
 */
...
const VALID_ROLES: readonly UserRole[] = ['super', 'team'];
const DEFAULT_MIN_PASSWORD_LENGTH = 8;
const LOCAL_DEV_MIN_PASSWORD_LENGTH = 7;

/**
 * The normal account policy remains 8+ characters. `run-dev` explicitly opts into the
 * seven-character floor because the local development seed password is seven characters.
 */
export function passwordValidationError(password: string, allowShortPassword = false): string | null {
  const minLength = allowShortPassword ? LOCAL_DEV_MIN_PASSWORD_LENGTH : DEFAULT_MIN_PASSWORD_LENGTH;
  return password.length < minLength ? `Password must be at least ${minLength} characters` : null;
}
...
async function create(args: string[]): Promise<number> {
  ...
  console.log(heading(`greenhouse — create ${roleFlag} user`));
  ...
  const allowShortPassword = flagBool(flags, 'allow-short-password');
  ...
  const minPasswordLength = allowShortPassword ? LOCAL_DEV_MIN_PASSWORD_LENGTH : DEFAULT_MIN_PASSWORD_LENGTH;
  if (!password) password = (await prompt(`Password (min ${minPasswordLength} chars): `)).trim();
  const passwordError = passwordValidationError(password, allowShortPassword);
  if (passwordError) {
    console.error(chalk.red(`❌ ${passwordError}`));
    return 1;
  }
