...
- 上游 env 为 `MEDIA_API_KEY`/`MEDIA_BASE_URL`（见 `llm/media-provider.ts`，未配置时回落 `LLM_API_KEY`/`LLM_BASE_URL`，识图与生图共用一个 OpenAI 兼容端点）。响应可能**同时带 `b64_json` 和 `url`**，`persistImage` 优先取 base64：既少一跳，也不依赖上游托管 URL 从服务器可达。
...
### Persistent coworkers (2026-09-17)

- `coworkers/identity.ts` materializes one coworker per custom Profile asset and one default Sprouty per internal user. Profiles/version governance remains authoritative; instances never grant creator permissions. `sessions.agent_instance_id` is server-owned; legacy human sessions bind on their next turn. Public session metadata cannot forge execution lineage.
- `agent_chat` is a built-in, session-only tool: discover authorized colleagues, send a message, continue by dialogue ID. Bilateral state belongs to two coworkers, scoped to the initiating session/requester; each reply reuses the durable Subagent Runtime. Six rounds, twelve peer steps, existing ten-minute timeout and user budgets. Target access is checked every round and again on claim. No recursive dispatch, arbitrary session history, private human memories or writes in peer runs. Human chat retains its original identity.
- `coworker_dialogues`/`coworker_rounds` retain the real exchange; admission serializes per dialogue and deduplicates by tool-call ID. Failed/abandoned rounds are never automatically replayed. Internal dialogue sessions are hidden from default history and reject human chat mutations.
- Custom coworker memory is `requesting user × instance`; legacy/default Sprouty memories keep the null scope. The owner's memory UI shows the coworker label. Weekly consolidation currently processes only the legacy/default scope; decay applies to every scope. Mission workspaces are reused through `coworker_workspaces` for the same requester/instance, never across users. Explicit Mission workspace selections retain their existing owner checks.
- `/api/coworkers` is strictly the caller's inbox, including for super: list, open authorized profile, cursor-paged `/:id/topics`, `/:id/visit`, and `/topics/:id/read` (at most 100 observed assistant IDs). Historical topics are grouped by stable identity, not copied or concatenated into model context. Migration 0010 binds known historical human profiles and establishes old replies as read; future deliveries are unread until observed. Deleted/eval/internal topics are excluded.
- Inbox work/attention comes from existing Runtime runs, pending interrupts and unread failure notifications for that caller. Tool steps, bilateral peer turns and notification mirrors never increment chat unread counts. Reading a reply does not resolve an interrupt. Team still cannot see workflow activity. `/api/sessions?profile_key=...` applies the same stable-profile filter to normal and organized backfill rows; private filtered views never backfill foreign chats.
