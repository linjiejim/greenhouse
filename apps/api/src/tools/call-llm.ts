...
const CALL_LLM_MAX_OUTPUT_TOKENS = 4_096;
const CALL_LLM_MAX_RETRIES = 3;
...
const defaultGenerate: OneShotGenerate = async ({
  modelConfig,
  system,
  prompt,
  maxOutputTokens,
    ...
    execute: async (input: CallLlmInput, { abortSignal: parentSignal }: { abortSignal?: AbortSignal } = {}) => {
      ...
      try {
        const providerAttemptHook = createProviderAttemptBudgetHook({
          ...
          runId: ctx.sessionId,
          metadata: { session_id: ctx.sessionId, profile_id: profile.id },
        });
        ...
        if (ctx.generate) {
          injectedEstimate = estimateCompletionTokens({
            system: input.system ?? '',
            messages: [{ role: 'user', content: input.prompt }],
            maxOutputTokens: CALL_LLM_MAX_OUTPUT_TOKENS,
            maxRetries: CALL_LLM_MAX_RETRIES,
          });
          injectedBudget = await reserveUserTokenBudget({
            ...
            modelId: modelLabel,
            runId: ctx.sessionId,
            metadata: { session_id: ctx.sessionId, profile_id: profile.id, injected_generator: true },
          });
        ...
        if (injectedBudget) {
          try {
            await settleAndRecordBudgetedUsage(db, injectedBudget, {
              ...
              output_tokens: result.usage ? (result.usage.outputTokens ?? 0) : injectedEstimate,
              cached_tokens: 0,
              reasoning_tokens: 0,
              duration_ms: durationMs,
            });
          } catch (err) {
            logger.warn('[call_llm] failed to settle injected-generator usage', {
              sessionId: ctx.sessionId,
              budgetKey: injectedBudget.idempotencyKey,
